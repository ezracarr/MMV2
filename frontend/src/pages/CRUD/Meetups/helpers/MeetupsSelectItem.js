...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/meetups/autocomplete`, { params });
  return response.data;
}

const MeetupsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.meetups.hasPermissionToCreate,
});

export default connect(select)(MeetupsSelectItem);
