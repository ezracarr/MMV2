...
const MeetupsTable = () => {
  ...
  const count = useSelector((store) => store.meetups.list.count);
  const modalOpen = useSelector((store) => store.meetups.list.modalOpen);
  const rows = useSelector((store) => store.meetups.list.rows);
  const idToDelete = useSelector((store) => store.meetups.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>Meetups</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/meetups/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Meetups'
                : 'http://localhost:8080/api-docs/#/Meetups'
            }
          >
            API documentation for meetups
          </LinkMaterial>
        </div>
      </Widget>
