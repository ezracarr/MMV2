...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/nodes/autocomplete`, { params });
  return response.data;
}

const NodesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.nodes.hasPermissionToCreate,
});

export default connect(select)(NodesSelectItem);
