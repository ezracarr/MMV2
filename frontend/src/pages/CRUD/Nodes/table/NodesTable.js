...
>>>>>>> up_and_running
  ...
  const count = useSelector((store) => store.nodes.list.count);
  const modalOpen = useSelector((store) => store.nodes.list.modalOpen);
  const rows = useSelector((store) => store.nodes.list.rows);
  const idToDelete = useSelector((store) => store.nodes.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>Nodes</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/nodes/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Nodes'
                : 'http://localhost:8080/api-docs/#/Nodes'
            }
          >
