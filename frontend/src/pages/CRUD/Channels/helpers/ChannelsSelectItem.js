...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/channels/autocomplete`, { params });
  return response.data;
}

const ChannelsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.channels.hasPermissionToCreate,
});

export default connect(select)(ChannelsSelectItem);
