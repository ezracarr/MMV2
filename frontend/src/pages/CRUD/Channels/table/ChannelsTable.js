...
import * as dataFormat from 'pages/CRUD/Channels/table/ChannelsDataFormatters';

import actions from 'actions/channels/channelsListActions';
...
const ChannelsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Party A', title: 'partyA' },
    { label: 'Party B', title: 'partyB' },
    { label: 'Party A Local Balance', title: 'partyA_local_balance' },
    { label: 'Party B Local Balance', title: 'partyB_local_balance' },
  ]);
  ...
  const count = useSelector((store) => store.channels.list.count);
  const modalOpen = useSelector((store) => store.channels.list.modalOpen);
  const rows = useSelector((store) => store.channels.list.rows);
  const idToDelete = useSelector((store) => store.channels.list.idToDelete);
  ...
  const columns = [
    ...
    {
      field: 'partyA_local_balance',

      flex: 0.6,

      headerName: 'Party A Local Balance',
    },

    {
      field: 'partyB_local_balance',

      flex: 0.6,

      headerName: 'Party B Local Balance',
    },
  ...
  return (
    <div>
      <Widget title={<h4>Channels</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/channels/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Channels'
                : 'http://localhost:8080/api-docs/#/Channels'
            }
          >
