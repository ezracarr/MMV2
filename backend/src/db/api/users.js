...
module.exports = class UsersDBApi {
  static async create(data, options) {
    ...
    await users.setMeetups(data.data.meetups || [], {
      transaction,
    });
  ...
  static async update(id, data, options) {
    ...
    await users.setMeetups(data.meetups || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.meetups = await users.getMeetups({
      transaction,
    });
    ...
    let include = [
      {
        model: db.meetups,
        as: 'meetups',
        through: filter.meetups
          ? {
              where: {
                [Op.or]: filter.meetups.split('|').map((item) => {
                  return { ['Id']: Utils.uuid(item) };
                }),
              },
            }
          : null,
        required: filter.meetups ? true : null,
      },
